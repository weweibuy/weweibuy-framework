...
(
   ...
   compensate_state     varchar(20) not null comment '补偿状态(RETRY_ABLE:可以重试; ALARM_ABLE:可以报警; OVER_ALARM_COUNT:超出报警次数)',
   compensate_result    varchar(20) not null comment '补偿结果: (RETRY_FAIL:补偿重试失败; RETRY_FAIL_RECOVER:补偿重试失败,触发恢复方法; RETRY_SUCCESS:补偿重试成功; ALARM:报警; OVER_ALARM_COUNT:超出报警上限)',
